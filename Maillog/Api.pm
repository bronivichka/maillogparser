...
# Самый простой HTTP::Daemon
# Принимает POST запросы по адресу http://host:port/search
# в форме ищет поле address и выполняет минимальную его проверку
# Выполняет поиск в БД по заданному адресу и возвращаем результат
# результат в виде простых строк без форматирования
